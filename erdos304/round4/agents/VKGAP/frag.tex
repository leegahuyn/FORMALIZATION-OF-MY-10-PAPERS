...
\item (엔트로피 수준, DG) $G(D)=c\frac{D}{\log L}\log\frac{eL}{D}$ ($D\le L$에서 증가)이면
...
\item \COND{RH}\ (보조정리~\ref{VKGAP:lem-RH}) $T'=T_{\rm RH}(L)=\exp(c_{\rm RH}\sqrt L/\log L)$, $\eta=1/15$:
...
Erdős \#18 Q1 ($h(m)<(\log\log m)^{O(1)}$인 practical $m$이 무한히 많음)이 강한 형태 (지수 2, $m=\Lam_L$)로 따른다.
...
즉 범위를 $|\tau|\ge1$에서 시작하는 비공명은 $\eta>0.0194$에서 \REFUTED{} (큰 $L$). 반면 $\sup_{|\tau|\ge4}|F|\le3/\sqrt{17}=0.7276$이므로
...
$|u|\le 2|\tau|+y^3$이므로 $|G|\le Z_{\rm RH}:=C'\log y\log(|\tau|+y)$. 왼쪽 변 $\le\frac{36}{2\pi}y^{\sigma_1}Z_{\rm RH}=\frac{36e}{2\pi}\sqrt yZ_{\rm RH}$,
수평 변 $\le26Z_{\rm RH}y^{-4}$, 꼬리 $\le16/y$, 소수 거듭제곱 $\le3\sqrt y$. 합하면 첫 주장.
...
$\mathrm{Var}_P(\log p)=0.03937,0.03932,0.03916$ (극한 $0.03907$; 증명은 $v_0=0.0059$만 사용);
...
Prop.~\ref{VKGAP:prop-core} & $\mathrm{NR}'$ $\Rightarrow$ 로그합 조밀성 (Fejér 국소 극한, 명시 상수) & \PROVED\\
...
Rem.~\ref{VKGAP:rem-cutoff} & $|\tau|\ge1$부터의 NR은 $\eta>0.0194$에서 거짓 & \REFUTED{} (+(E1))\\
