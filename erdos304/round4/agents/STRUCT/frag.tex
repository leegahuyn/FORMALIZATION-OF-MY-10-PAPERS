...
(1) \PROVED\ 정리~\ref{STRUCT:thm-regimes}(C)의 $\log L\log\log L$은 순수 greedy가 결코 넘을 수 없는 선이다 (따름정리~\ref{STRUCT:cor-greedy-theta}(i)); 명제~\ref{STRUCT:prop-DG-sharp}에 의해 어떤 간격 가설도 greedy 단계당 $(1+o(1))\phi_L(\ell)$보다 많이 $\log R$을 줄일 수 없다. 따라서 SP1 ($H\ll\log L$)의 증명은 greedy가 아닌 다중 약수 단계를 포함해야 하고, S2의 정리~\ref{STRUCT:thm-barrier}에 의해 그 단계들은 유계 비율의 $\Lam$형 블록일 수도 없다: 넓은(초지수) 단계 또는 $\Lam$형이 아닌 사슬(풀 소수 설계, cascade)이 필요하다. ``필요하다''는 위 두 증명된 장벽의 결합이라는 뜻이며 모든 가능한 방식에 대한 불가능성 정리는 아니다 (\HEUR).
...
\end{proposition}
\begin{proof}
$C<K/(K-1)$이라 가정하면 $1-1/C<1/K$이므로 $1-1/C<\sigma<1/K$인 $\sigma$가 있다. Rankin: $\Psi(x,y)\le\sum_{P^+(d)\le y}(x/d)^\sigma=x^\sigma\prod_{p\le y}(1-p^{-\sigma})^{-1}$이고
