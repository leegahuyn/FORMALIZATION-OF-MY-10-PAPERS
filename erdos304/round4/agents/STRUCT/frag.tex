...
\emph{Rankin.} $\Psi_\Lam(X):=\#\{d\mid\Lam_L:d\le X\}\le\sum_{d\mid\Lam_L}(X/d)^\sigma=X^\sigma F(\sigma)$. $L\ge7$이면 $p\le\sqrt L$의 인수는 $\le a_p+1\le1+\log_2L\le2\log L$, $p>\sqrt L$의 인수는 $1+p^{-\sigma}\le\exp(p^{-\sigma})$이므로 $\log F(\sigma)\le\sqrt L\log(2\log L)+\sum_{p\le L}p^{-\sigma}$. 부분합과 $\pi(t)\le3t/\log t$ ($t\ge2$; Chebyshev형, 초등적), $\pi(L)\le(1+\eps_L)L/\log L$ (PNT, $\eps_L\to0$)로
