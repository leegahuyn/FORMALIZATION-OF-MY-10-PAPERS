...
\begin{table}[ht]\centering\small
%%HTABLE%%
\caption{정확한 $H(L)$, $L\le %%LMAX%%$. \NUMERIC\ (\texttt{code/hbfs.c}, \texttt{code/hbfs2.c}; \texttt{data/H\_table\_bfs.csv}, \texttt{data/bfs/}).
...
\HEUR\ $H$ 는 $L=17$ 부터 %%HLAST%% 까지 $7$ 에 머문다. $L=23$ 에서 $H/\ln L=2.23$ (범위 최댓값 $L=24$ 에서 $2.20$) 로
...
이동 항등식 $\kappa(\Lam/2+b)=1+\kappa(b)$ 는 $L\le %%SHIFTL%%$ 의 모든 $b$ 에서 성립하지만 일반 $L$ 에 대한 증명은 없다 \OPEN.
