\section{NUM: 다중집합 BFS 에 의한 정확한 $H(L)$ ($L\le 25$) 와 공명 높이 실험}\label{NUM:sec}
...
$L=25$ 는 메모리 한도(7\,GB) 때문에 \texttt{hbfs2} 로 계산하였다 (비트집합 3.35\,GB).
...
세 약수 합을 직접 나열한 $S_3=\{d_1+d_2+d_3<\Lam: d_i\in D^*\cup\{0\}\}$ 는 $|S_3|=80181647$ 이고 BFS 의 $R_3$ 와 비트 단위로 같다 (다른 비트 0개). $\kappa(a)\le6\iff a\in S_3+S_3$ 를 전수 대조하여 $a_7=3\,716\,552\,837$ 이 $S_3+S_3$ 에 없음 (즉 $\kappa(a_7)\ge7$) 을 BFS 와 독립으로 확인하였고, 표본 41개 (중복 포함 $\kappa=7$ 쪽 22개, $\kappa\le6$ 쪽 19개) 에서 BFS 의 $R_6$ 판정과 불일치 0개.
\end{enumerate}
\end{proposition}
...
\item 실행: \texttt{hbfs2} 2 스레드 609\,s, 메모리 4.4\,GB. 두 번째 구현 \texttt{hbfs} 에 의한 재계산은 끝나지 않았다 (\texttt{out/hbfs\_L25.log} 참조).
\end{enumerate}
\end{theorem}
...
173 & 17 & 3.42 & -- & 6.30 & -- & 8.87 & --\\
181 & 18 & 3.42 & -- & 6.34 & -- & $>9.10$ & --\\
191 & 19 & 4.87 & -- & 6.81 & -- & $>9.08$ & --\\
193 & 20 & 4.78 & -- & 6.81 & -- & 9.14 & --\\
199 & 21 & 4.78 & -- & 6.81 & -- & $>9.08$ & --\\
239 & 22 & 5.79 & -- & 8.41 & -- & $>9.08$ & --\\
241 & 23 & 6.20 & -- & 8.55 & -- & $>9.05$ & --\\
...
\item LLL 이 찾은 점은 인증된 공명 없는 구간 안에 한 번도 들어가지 않았다 (모순 없음). 두 값이 모두 있는 36 경우 중 35 경우에서
LLL 점은 격자의 첫 교차점과 $3$ 이내 (같은 공명 봉우리) 이다. 즉 이 범위에서 LLL 은 사실상 \emph{첫} 공명을 찾는다.
\item 최소제곱 기울기 (상용로그, 소수 하나당):
\begin{itemize}
\item $\eta=0.3$: 격자 $\tau^*$ (20개, $M\in[5,24]$) $\log_{10}\tau^*\approx0.68+0.212M$; LLL 점 (12개, $M\le16$) $\log_{10}t\approx0.85+0.201M$; 무작위 위상 모형 기울기 0.250
\item $\eta=0.2$: 격자 $\tau^*$ (20개, $M\in[5,24]$) $\log_{10}\tau^*\approx0.02+0.353M$; LLL 점 (12개, $M\le16$) $\log_{10}t\approx0.43+0.309M$; 무작위 위상 모형 기울기 0.354
\item $\eta=0.1$: 격자 $\tau^*$ (14개, $M\in[5,20]$) $\log_{10}\tau^*\approx-0.13+0.496M$; LLL 점 (12개, $M\le16$) $\log_{10}t\approx-0.33+0.518M$; 무작위 위상 모형 기울기 0.520
\end{itemize}
\item $\eta=0.1$ 에서 격자는 $M\ge18$ 부터 120\,s 안에 공명을 찾지 못한다: 인증된 하한 $\tau^*_{10}>1.11e+09$ (최소값).
\end{enumerate}
\end{proposition}
...
관측 기울기는 $\eta=0.2,0.1$ 에서 이 예측과 가깝고, $\eta=0.3$ 에서는 작다 (작은 $M$ 에서 공유 공명 --- 예: $L=53,61,109$ 가 거의 같은 높이 $\approx360$, $642$ 에서 공명 --- 과 유한 크기 효과).
